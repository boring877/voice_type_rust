    ...
    <section className="dock-section">
      ...
      <div className="voice-actions">
        <Toggle
          ...
          description="Fixes grammar and punctuation before the final text is pasted."
...
export function StyleSection(props: {
  ...
  const previewText =
    config.style === "japanese_emojis"
      ? "Example: (^_^) / (・o・)? / (≧▽≦)"
      : config.style === "niko_style"
        ? "Example: nya! / nya? / nya (=^･ω･^=)"
        : "Example: final text stays unchanged";
  ...
  return (
    <section className="style-panel card">
      ...
      <div className="style-panel-body">
        ...
        <p className="style-panel-preview">{previewText}</p>
    ...
    <section className="dock-section">
      ...
      <div className="capture-row">
        <button className="secondary-button" onClick={onStartHotkeyCapture} type="button">
          {capturingHotkey ? "Listening..." : "Record key"}
        </button>
        {capturingHotkey ? (
  ...
  return (
    <section className="dock-section">
      ...
      <div className="feature-grid">
        <Toggle
          label="Casual mode"
          description="Keeps the wording looser with lowercase text and lighter punctuation."
          checked={config.casual_mode}
          onChange={(value) => onUpdate("casual_mode", value)}
        />
        <Toggle
          label="Shorthand mode"
          description="Turns common phrases into chat shortcuts like to be honest to tbh."
          checked={config.shorthand_mode}
          onChange={(value) => onUpdate("shorthand_mode", value)}
        />
        <Toggle
          label="Accounting mode"
          description="Turns spoken number phrases like two million into numeric values."
          checked={config.accounting_mode}
          onChange={(value) => onUpdate("accounting_mode", value)}
        />
        <Toggle
          label="Accounting commas"
          description="Formats long numbers like 2000000 as 2,000,000 so they are easier to read."
          checked={config.accounting_comma}
          onChange={(value) => onUpdate("accounting_comma", value)}
        />
        <Toggle
          label="Auto-copy"
          description="Copies the final transcription to the clipboard automatically."
          checked={config.auto_copy}
          onChange={(value) => onUpdate("auto_copy", value)}
        />
        <Toggle
          label="Always on top"
          description="Keeps the app above your other windows while you work."
...
export function CleanupSection(props: {
  config: Config;
  ...
  return (
    <section className="dock-section">
      <SectionHeader title="Text Cleanup" />
